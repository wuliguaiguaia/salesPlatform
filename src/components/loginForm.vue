...
<div>
    <form class="dialog-form">
        <div class="dialog-form-line">
            ...
            <input type="text" id="username" placeholder="请输入用户名" v-model="username" autocomplete="on" autofocus>
            <span class="formError">{{nameError.errorText}}</span>
        </div>
        <div class="dialog-form-line">
            <label for="password">密码 ：</label>
            <input type="password" id="password" placeholder="请输入密码" v-model="password" autocomplete="on">
            <span  class="formError">{{passError.errorText}}</span>
        </div>
        <button type="" class="submit" @click="onsubmit">登录</button>
